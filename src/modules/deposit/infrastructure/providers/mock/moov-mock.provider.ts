...
@Injectable()
export class MoovMockProvider implements IPaymentProvider {
  private readonly logger = new Logger(MoovMockProvider.name);
  ...
  getProviderName(): string {
    return 'Moov Money Côte d\'Ivoire';
  }
  ...
  async initiateCharge(params: InitiateChargeParams): Promise<InitiateChargeResult> {
    this.logger.log(`Initiating Moov charge for ${params.amount} ${params.currency} to ${params.phoneNumber}`);
    ...
    const providerTransactionId = `moov_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    ...
    if (params.phoneNumber === '+2250700000006') {
      ...
      // Auto-confirm after 2 seconds for successful scenarios
      setTimeout(() => {
    ...
    return {
      ...
      instructions: 'Vous recevrez une notification Moov Money. Approuvez avec votre code PIN.',
      expiresAt,
    };
  }

  async confirmCharge(params: ConfirmChargeParams): Promise<ConfirmChargeResult> {
    this.logger.log(`Confirming Moov charge: ${params.providerTransactionId}`);
  ...
  async verifyTransaction(providerTransactionId: string): Promise<ConfirmChargeResult> {
    this.logger.log(`Verifying Moov transaction: ${providerTransactionId}`);
