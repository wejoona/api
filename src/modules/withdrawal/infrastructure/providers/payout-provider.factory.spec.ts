...
describe('PayoutProviderFactory', () => {
  ...
  it('rejects mock mode in production-like environments', () => {
    expect(() =>
      makeFactory({ nodeEnv: 'production', WITHDRAWAL_USE_MOCK: 'true' }),
    ).toThrow('WITHDRAWAL_USE_MOCK=true is not allowed');
  });

  it('returns explicit unavailable errors when live payout providers are requested', () => {
    const factory = makeFactory({
      nodeEnv: 'production',
      WITHDRAWAL_USE_MOCK: 'false',
    });
    ...
    try {
      ...
      expect(exception.code).toBe(ERROR_CODES.WITHDRAWAL_PROVIDER_UNAVAILABLE);
