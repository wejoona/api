...
/**
 * Migration: Add Circle integration fields and new tables
 *
 * This migration adds:
 * 1. Circle fields to users table (circle_user_id, circle_user_token)
 * 2. Circle fields to wallets table (circle_wallet_id, circle_wallet_address)
 * 3. Ledger tables (ledger_accounts, ledger_entries, ledger_transactions)
 * 4. Transfers table
 * 5. Notifications and device_tokens tables
 */
export class AddCircleFieldsAndNewTables1737300000000 implements MigrationInterface {
  name = 'AddCircleFieldsAndNewTables1737300000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // ======================================
    // 1. Add Circle fields to users table
    // ======================================
    ...
    // ======================================
    // 2. Add Circle fields to wallets table
    // ======================================
    await queryRunner.query(`
      ALTER TABLE "wallets"
    ...
    // Update default currency from USD to USDC
    await queryRunner.query(`
      ALTER TABLE "wallets" ALTER COLUMN "currency" SET DEFAULT 'USDC'
    `);

    // ======================================
    // 3. Create ledger_accounts table
    // ======================================
    await queryRunner.query(`
    ...
    // ======================================
    // 7. Create device_tokens table (for push notifications)
    // ======================================
    await queryRunner.query(`
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ledger_transaction_status_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ledger_transaction_type_enum"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "ledger_account_category_enum"`,
    );
    await queryRunner.query(`DROP TYPE IF EXISTS "ledger_account_type_enum"`);

    // Remove Circle columns from wallets
    await queryRunner.query(
    ...
    // Remove Circle columns from users
